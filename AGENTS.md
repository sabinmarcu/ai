...
- In this repository, "AI files" means local AI files in the target project and not catalog modules.
- Treat modules and local AI files as separate destinations with different lifecycle rules.
...
- Detector-confirmed effective modules: `guidance/conventional-commits`, `global/core`, `arch/node-package`, `arch/node-package-library`, `arch/node-library`, `arch/node-root-package`, `arch/node-package-application`, `arch/node-tool`, `lang/typescript`, `tooling/commitlint`, `tooling/lint-staged`, `tooling/eslint`, `tooling/husky`, and `tooling/yarn`.
