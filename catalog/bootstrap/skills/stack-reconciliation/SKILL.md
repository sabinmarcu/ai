...
description: "Inspect repository changes for AI stack applicability, review module and mixin changes, and reconcile managed AI assets through the ai-lib CLI."
user-invocable: true
...
1. Run `ai-lib reconcile` to inspect fresh repository and workspace detection,
...
3. Ask for approval before changing explicit module selections or repairing
   drift.
4. After approval, run `ai-lib reconcile --apply`. Do not edit `.ai/stack.yml`,
   managed files, or `.ai/AGENTS.md` directly.
5. Run `ai-lib status` and report the resulting validation state. Leave failed
